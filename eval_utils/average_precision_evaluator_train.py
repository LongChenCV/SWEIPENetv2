...
class Evaluator:
    def __init__(self,
        ...
        self.model = model
        self.stage = stage
        self.sample_weights_dir = sample_weights_dir
    ...
    def predict_on_dataset(self,
        ...
        overallindex=0 # overallindex is the total number of gt boxes
        # For all gt boxes, save as whether they are detected or not, imname+four locations+detected/undetected
        dettxt=os.path.join(self.sample_weights_dir,'detections.txt')
        if not os.path.exists(dettxt):
            os.mknod(dettxt)
        fid=open(dettxt, 'w')
        for j in tr:
            # Generate batch.
            ...
            for k, batch_item in enumerate(y_pred):
                ...
                detspath=os.path.join(self.sample_weights_dir, 'Detections')
                if not os.path.exists(detspath):
                    os.mkdir(detspath)
                txtpath = os.path.join(detspath, image_id + '.txt')
                if not os.path.exists(txtpath):
                    os.mknod(txtpath)
                file_fid = open(txtpath, 'w')

                predictions=[] # save all the location of predictions
                for box in batch_item:
                    ...
                    # save all the detections for the each class in class.txt, the detection include imname and four locations
                    results[class_id].append(prediction)

                file_fid.close()
                # groud truth class and location for the data in a batch
                gtlabels = self.data_generator.labels[overallindex]
                gtlabels=np.array(gtlabels)
                gtlabels=gtlabels[:, 2:]  # extract the locations
                overallindex=overallindex+1 # overallindex is the total number of gt boxes
        ...
        fid.close()
        # The Multi-class Adaboost algorithm
        self.Updateweight(gtlabels, predictions)
        print('Finishing update the weight!!!!!!!')

    def Updateweight(self, gtlabels, predictions):
        detfid = open(os.path.join(self.sample_weights_dir, 'detections.txt'), 'r')
        weightfid = open(os.path.join(self.sample_weights_dir, 'weights.txt'), 'r+')
        # read detection information:detected or undetected for each gt box
        ydet = [line.strip().split(',') for line in detfid]
        # ydetobj = [t[0] for t in ydet] # four location
        ydetlabel = [int(t[1]) for t in ydet] # 1:detected, 0: undetected
        detfid.close()
        ywei = [line.strip().split(',') for line in weightfid] # read previous weight for each gt box
        yweiobj = [t[0] for t in ywei]
        yweivalue = [float(t[1]) for t in ywei]
        weightfid.close()
        objnum = len(yweiobj) # total number of gt box in training set
        # label is True means detetecd
        label = ydetlabel == np.ones(objnum)
        estimator_error = np.dot(~label, yweivalue) / np.sum(yweivalue, axis=0)
        # update estimator_weight
        estimator_weight = np.log((1 - estimator_error) / estimator_error) + np.log(self.n_classes - 1)
        # convert SWIPENET weights into CMA weights
        weight_sum = np.sum(yweivalue, axis=0)
        yweivalue /= weight_sum # yweivalue now is the CMA weights, sum(yweivalue)=1
        # update sample weight using CMA algorithm
        if self.stage=='NECMA':
            # Downweighted the undected/noisy data and Upweighted the detected/easy samples in NECMA stage
            yweivalue *= np.exp(estimator_weight * label)
        else:
            # Upweighted the undected/noisy data and Downweighted the detected/easy samples in NLCMA stage
            yweivalue *= np.exp(~label*estimator_weight)
        sample_weight_sum = np.sum(yweivalue, axis=0)
        # normalize sample weight
        yweivalue /= sample_weight_sum
        yweivalue = yweivalue * objnum # convert CMA weights back into SWIPENET weights
        newweightfid = open(os.path.join(self.sample_weights_dir, 'weights.txt'), 'w')
        for weighindex in range(len(yweiobj)):
            newweightfid.writelines(yweiobj[weighindex]+', '+str("{:.4f}".format(yweivalue[weighindex]))+'\n')
        newweightfid.close()

    def CheckDetected(self, gtlabels=None, predictions=None):
        detect=[]
