...
class Evaluator:
    ...
    def Updateweight(self, gtlabels, predictions):
        ...
        # update sample weight, yweivalue now is the IMA weights, sum=1
        yweivalue *= np.exp(estimator_weight * label)
        sample_weight_sum = np.sum(yweivalue, axis=0)
        # normalize sample weight
        yweivalue /= sample_weight_sum
        yweivalue = yweivalue * objnum # convert IMA weights into SWIPENET weights, sum=72 /data/deeplearn/SWEIPENet/dataset
